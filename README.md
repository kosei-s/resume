...
大学院(情報系)を卒業して以来4年間ほど、主にデータ分析・基盤周辺の業務に携わってきました。主な業務内容としては、Hadoop/EMRを用いた分散並列処理、Pythonを用いたデータ加工・集計、BigQuery/Redshiftを用いたデータ集計、Airflowを用いたETLワークフロー構築等の経験があります。役割は基本的に設計・実装・テストです。
